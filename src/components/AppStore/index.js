...
class AppStore extends Component {
  state = {
    categoryAppList: appsList.filter(eachApp => eachApp.category === 'SOCIAL'),
    searchInputValue: '',
  }
  ...
  clickOnTab = tabId => {
    ...
    this.setState({categoryAppList: filteredAppList})
  }

  render() {
    const {categoryAppList, searchInputValue} = this.state

    const filteredCategoryList = categoryAppList.filter(eachList =>
      eachList.appName.toLowerCase().includes(searchInputValue.toLowerCase()),
    )
        ...
        <ul className="tabs-list">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              isActive={eachTab.tabId === categoryAppList[0].category}
            />
          ))}
        </ul>

        <ul className="apps-container">
          {filteredCategoryList.map(eachApp => (
